...
proc coderunner() =
  var loadnum = 0
  var lastcompile = getFileInfo("codecells.nim").lastWriteTime.toUnix
  newCode()
  while true:
    if getFileInfo("codecells.nim").lastWriteTime.toUnix-lastcompile > 1: 
      echo "# RELOAD PERFORMED ", loadnum
      performCodeReload()
      inc loadnum
      lastcompile = getFileInfo("codecells.nim").lastWriteTime.toUnix
...
--hotcodereloading:on
-d:jupyter
""" % [ (getHomeDir()/"inimtemp"/"nimcache").escapeJson ]

const codecellsinit = ["# generated by jupyternim \n", "echo \"initial codecell\"\n",]

proc genCodeFile(codecells: seq[string]): string =
  result = ""
  for i, cell in codecells:
    if i==codecells.len-1: # wrap the last cell in aftercodereload:
      result.add("proc newCode* ()=\n")
      result.add(cell.indent(2)) # indent to avoid compilation errors
    else:
      result.add(cell & "\n")

proc compileCodeServer(shell: var Shell) =
  if not fileExists(getHomeDir()/"inimtemp"/"nim.cfg"):
    debug "Write config"
    writeFile(getHomeDir()/"inimtemp"/"nim.cfg", nimcfg)

  if not fileExists(getHomeDir()/"inimtemp"/"codeserver.nim"):
    debug "Write out codeserver"
    writeFile(getHomeDir()/"inimtemp"/"codeserver.nim", codeserver) #FIXME:
    writeFile(getHomeDir()/"inimtemp"/"codecells.nim", genCodeFile(shell.codecells))
    echo execProcess(r"nim c --hotcodereloading:on " & getHomeDir() / "inimtemp" / "codeserver.nim") # compile the codeserver
  
  if not fileExists(getHomeDir()/"inimtemp"/"codeserver.exe"):
    debug "Compile codeserver"
    echo execProcess(r"nim c --hotcodereloading:on " & getHomeDir() / "inimtemp" / "codeserver.nim") # compile the codeserver
  
proc startCodeServer(shell: var Shell): Process =
  debug "confirm codeserver.exe exists: ", fileExists(getHomeDir()/"inimtemp"/"codeserver.exe")
  result = startProcess(getHomeDir() / "inimtemp" / "codeserver.exe") # TODO: fix for non windows, ensure codeserver exists  
...
proc createShell*(ip: string, shellport: BiggestInt, key: string,
  ...
  result.codecells = @codecellsinit
  result.compileCodeServer()
...
var flags: seq[string] = @["-d:debug"] # Default flags, will be overwritten if others are passed
...
proc handleExecute(shell: var Shell, msg: WireMessage) =
  ...
  if code.contains("#>clear all") and dirExists(getHomeDir() / "inimtemp"):
    ...
    shell.codecells = @codecellsinit #reset code
  
  let srcfile = getHomeDir() / "inimtemp" / "codecells.nim"
  writeFile(srcfile, genCodeFile(shell.codecells)) # write out the logic file
  ...
  # TODO: handle flags
  var compiler_out = execProcess("nim c " & flatten(flags) & " -d:jupyter --hotcodereloading:on " & getHomeDir() / "inimtemp" / "codeserver.nim") # compile block
  
  # debug "nim c " & flatten(flags) & " -d:jupyter --hotcodereloading:on " & getHomeDir() / "inimtemp" / "codeserver.nim"
  ...
  if compiler_out.contains("Error:"):
    status = "error"
    std_type = "stderr"
    #shell.codeserver = startCodeServer(shell) # restart codeserver, maybe dead?
  ...
  if status == "error" or status == "abort":
    ...
    debug "trying to read all..."
    while not shell.codeserver.hasData: 
      sleep(100)
      continue
    var exec_out = shell.codeserver.outputStream.readAll
    debug "done??: ", exec_out
  ...
  if status == "error" or status == "abort":
    ...
    discard shell.codecells.pop() #TODO: avoid the discard by just setting codecells.len?
  else:
    content = %* {
      "status": status,
      "execution_count": shell.count,
