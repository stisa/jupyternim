...
var flags: seq[string] = @["-d:debug", "--hotcodereloading:on"] # Default flags, will be overwritten if others are passed
...
proc genCodeFile(codecells: seq[string]): string =
  ...
  for i, cell in codecells:
    if i==codecells.len-1: # wrap the last cell in aftercodereload:
      ...
      #result.add("afterCodeReload:\n")
      result.add(cell.indent(2)) # indent to avoid compilation errors
      #result.add(cell&"\n")
    else:
      result.add(cell & "\n")

proc setupCodeServer(shell: var Shell) =
  #debug "Write config"
...
proc startCodeServer(shell: var Shell): Process =
  ...
  echo execProcess(r"nim c " & flatten(flags) & getHomeDir() / "inimtemp" / "codeserver.nim") # compile the codeserver
  ...
  sleep(1000) # maybe waiting will fix this?

  if result.hasData:
    echo "from startCodeServer: ", result.outputStream.readAll
  else:
    echo "result didnt have data"
...
proc createShell*(ip: string, shellport: BiggestInt, key: string,
  ...
  result.setupCodeServer()
  result.codeserver = result.startCodeServer()
  result.serverout = result.codeserver.outputStream
...
proc handleExecute(shell: var Shell, msg: WireMessage) =
  ...
  if code.contains("#>clear all") and dirExists(getHomeDir() / "inimtemp"):
    debug "Cleaning up..."
    flags = @["-d:debug", "--hotcodereloading:on"]
  ...
  # TODO: handle flags
  var compiler_out = execProcess("nim c " & flatten(flags) & getHomeDir() / "inimtemp" / "codeserver.nim") # compile block
