...
when defined useHcr:
  const initCodecells = [staticRead("initialCodeCell.nim"), "discard \"Generated by JupyterNim\""]
else:
...
when not defined(release):
  flags[^3] = "-d:debug" #switch release to debug for the compiled file too
...
proc updateCodeServer(shell: var Shell, firstInit=false): tuple[output: string, exitcode: int] =
  ...
  when defined useHcr:
    ...
    result = execCmdEx(r"nim c " & flatten(flags) & jnTempDir / "codecells.nim") # compile the codeserver
